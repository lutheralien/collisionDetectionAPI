...
def process_complete_video(input_path: str, output_path: str, timestamp: str):
    ...
    try:
        ...
        # Create video writer with mp4v codec first
        ...
        if not out.isOpened():
            raise ValueError(f"Failed to create output video writer at {temp_output}")
        ...
        try:
            ...
            # Convert to web-compatible format (H.264 codec)
            stream = ffmpeg.input(temp_output)
            stream = ffmpeg.output(stream, output_path, 
                                 vcodec='libx264',  # Use H.264 codec
                                 acodec='aac',      # Use AAC for audio if present
                                 preset='medium',    # Encoding preset
                                 movflags='faststart',  # Enable fast start for web playback
                                 **{'brand': 'mp42'})   # Use compatible MP4 brand
            ...
            logger.warning("FFmpeg Python bindings not found, falling back to command line FFmpeg")
            import subprocess
            ...
            try:
                ...
                raise ValueError("Error converting video to web-compatible format")
